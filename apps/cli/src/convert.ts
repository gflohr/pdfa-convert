...
import { decodePDFRawStream, PDFArray, PDFDict, PDFDocument, PDFName, PDFObject, PDFRawStream, type PDFPage } from '@cantoo/pdf-lib';
...
export class ContentStreamParser {
	...
	parseDocument(pdfDoc: PDFDocument) {
		...
		this.parseRecursively(pdfDoc, pdfDoc);
	}
	...
	parseStream(stream: PDFRawStream) {
			...
			if (state === 'initial') {
				...
				if (byte === 62) {
					...
					for (let j = 0; j < token.value.length - 1; j += 2) {
						value.push(16 * this.hexCharToNumber(token.value[j]) + this.hexCharToNumber(token.value[j + 1]));
					}
					token.value = [...value];
					tokens.push(token);
					token = { type: 'token', value: [] };
					state = 'initial';
						...
						|| (byte >= 65 && byte <= 70) || (byte >= 97 && byte <= 102)) {
					token.value.push(byte);
				}
			}
		}
		...
		for (let i = 1; i < tokens.length; ++i) {
			token = tokens[i];
			if (token.type === 'token' && token.value.length === 2 && token.value[0] === 84 && token.value[1] === 106) {
