...
describe('Load Font', () => {
	...
	it('should load a single TrueType font', async () => {
		const bytes = await fs.readFile(
			path.resolve(import.meta.dirname, './data/OpenSans/OpenSans-Regular.ttf'),
		);
	...
	it('should load a single WOFF2 font', async () => {
		const bytes = await fs.readFile(
			path.resolve(import.meta.dirname, './data/SourceSansPro/SourceSansPro-Regular.woff2'),
		);
		const font = fontkit.loadFont(bytes);
		expect(font).toBeDefined();
	...
	it('should load a single WOFF font', async () => {
		const bytes = await fs.readFile(
			path.resolve(import.meta.dirname, './data/SourceSansPro/SourceSansPro-Regular.woff'),
		);
		const font = fontkit.loadFont(bytes);
		expect(font).toBeDefined();
	...
	it('should throw an error if PostScript name is passed for a font', async () => {
		const bytes = await fs.readFile(
			path.resolve(import.meta.dirname, './data/SourceSansPro/SourceSansPro-Regular.otf'),
		);
		const postscriptName = 'SourceSansPro-Regular';

		const errorMessage = "Not a font collection!";
		expect(() => fontkit.loadFont(bytes, postscriptName)).toThrow(errorMessage);
	});
	...
	it('should throw an error if font is missing in collection', async () => {
		...
		const errorMessage = "Font collection does not contain 'NotoSans-Slanted'!"
