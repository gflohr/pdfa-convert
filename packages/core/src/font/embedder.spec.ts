...
describe('FontEmbedder', () => {
	...
	describe('TrueType Fonts', () => {
		const pdfFilename = path.resolve(pdfDir, 'truetype-missing.pdf');
		let lab: PDFALab;

		beforeAll(async () => {
			const pdfBytes = await fs.readFile(pdfFilename);
			const trueTypeLab = await PDFALab.from(pdfBytes);
			...
			const embeddedBytes = await trueTypeLab.save();
			lab = await PDFALab.from(embeddedBytes);
		});

		it('should embed the 2 TrueType fonts', async () => {
			const fonts = [...lab.collectFonts().values()];

			expect(fonts.length).toBe(2);
			expect(fonts.filter((font) => font.embedded).length).toBe(2);
			expect(fonts.filter((font) => font.subtype === 'Type0').length).toBe(2);
	...
	describe('Type0 Fonts', () => {
		...
		beforeAll(async () => {
			const pdfBytes = await fs.readFile(pdfFilename);
			const type0Lab = await PDFALab.from(pdfBytes);
