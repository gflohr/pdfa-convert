...
* **Static Format Identifiers:** Font classes and font collections expose a
  `static readonly type` (e.g., `WOFF2Font.type === font.type`) matching
  instance `.type` properties to enable type-safe, autocomplete-friendly format
  comparisons without string literal guessing.
